...
public class TestHarness {
    ...
    public TestHarness(File commonDir, File testCaseDir, File externalDir, String testName) {
        ...
        this.testClassName = testName(testCaseDir, testName);
    }

    // TODO Always load Test.java
    private static String testName(File testCaseDir, String testName) {
        List<String> files = Arrays.asList(testCaseDir.list());
        boolean isErrorCase = testCaseDir.getName().startsWith("E");
        boolean hasTest2 = files.contains("Test2.java");
        Assume.assumeFalse(isErrorCase && !hasTest2);
        String simpleClassName = hasTest2 ? "Test2" : "Test";
        return "testcases." + testName + "." + simpleClassName;
    }
    ...
    public void test() throws Throwable {
        ...
        if (compilation.status() == Compilation.Status.FAILURE) {
            Compilation noProcessorCompilation = compiler.compile(
                    ...
                    null,
                    new StubProcessor(),
                    testCaseDir).compilation;
            assertThat(noProcessorCompilation).succeeded();
        ...
        try {
            Field expected = testClass.getField("errors");
            List<ProcessingError> errors = processor.graph.getProcessingErrors();
            Truth.assertWithMessage("No ProcessingErrors found.").that(errors).isNotEmpty();
            expected.set(null, errors);
